  ...
  return (
    <>
      <Card className="shadow-sm border-border/50 overflow-hidden">
        ...
        {/* ── Pagination ── */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-3 border-t border-border/40 bg-muted/5">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            >
              <SelectTrigger className="w-16 h-8 text-black bg-white border border-border">
...
function renderViewValue(record: RecordRow, field: Field) {
  ...
  switch (field.field_type) {
    ...
    case 'Email':
      return <a href={`mailto:${value}`} className="text-blue-600 hover:underline break-all">{String(value)}</a>;
    case 'MobileNumber':
      return <a href={`tel:${value}`} className="text-blue-600 hover:underline break-all">{String(value)}</a>;
    case 'UrlLink':
    case 'SocialLink':
      return <a href={String(value)} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline break-all">{String(value)}</a>;
    default:
      return <span className="break-all">{String(value)}</span>;
  }
}
